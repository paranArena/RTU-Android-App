...
class CouponListAdapter internal constructor(var couponList: List<CouponListModel>, val type: Int)
    : RecyclerView.Adapter<CouponListAdapter.ListViewHolder>() {


    inner class ListViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(_list: CouponListModel) {
            ...
            val actAt=_list.actDate.substring(0 until 10).replace("-",". ")
            val expAt=_list.expDate.substring(0 until 10).replace("-",". ")

            itemView.findViewById<TextView>(R.id.iv_date).text = "$actAt ~ $expAt"

            if(type==1){
                itemView.findViewById<TextView>(R.id.used).text = "미사용"
            } else{
                itemView.findViewById<TextView>(R.id.used).text = "사용"
            }
        }
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ListViewHolder {
        return ListViewHolder(
            ...
            LayoutInflater.from(parent.context).inflate(R.layout.coupon_list, parent, false)
        )
    }
